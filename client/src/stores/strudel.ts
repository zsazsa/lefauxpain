...
const [viewingPattern, _setViewingPattern] = createSignal(false);

// Wrap setViewingPattern so hiding the editor sends strudel_close
const setViewingPattern = (v: boolean) => {
  _setViewingPattern(v);
  if (!v && activePatternId()) {
    send("strudel_close", {});
