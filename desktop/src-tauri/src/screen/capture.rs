...
const FRAME_DURATION: Duration = Duration::from_millis(67); // ~15 FPS
...
async fn run_capture(
    ...
    let (frame_tx, mut frame_rx) = tokio::sync::mpsc::channel::<FrameData>(2);
...
/// Convert BGRA pixels to I420 (YUV420p) using BT.601 coefficients.
    ...
    for row in 0..height {
        for col in 0..width {
            ...
            y_plane[row * width + col] =
                (16.0 + 65.481 * r / 255.0 + 128.553 * g / 255.0 + 24.966 * b / 255.0) as u8;
        }
    }
        ...
        for col in 0..uv_width {
            ...
            for dy in 0..2 {
                for dx in 0..2 {
                    ...
                    b_sum += bgra[px] as u32;
                    g_sum += bgra[px + 1] as u32;
                    r_sum += bgra[px + 2] as u32;
                }
            }
            let r = (r_sum / 4) as f32;
            let g = (g_sum / 4) as f32;
            let b = (b_sum / 4) as f32;
            u_plane[row * uv_width + col] =
                (128.0 - 37.797 * r / 255.0 - 74.203 * g / 255.0 + 112.0 * b / 255.0) as u8;
            v_plane[row * uv_width + col] =
                (128.0 + 112.0 * r / 255.0 - 93.786 * g / 255.0 - 18.214 * b / 255.0) as u8;
        }
    }
...
/// Convert RGBA pixels to I420 (YUV420p) using BT.601 coefficients.
